...
// 		echo($count_sql);
		if ($count > 0 ) {
		    ...
		    //五倍开奖
        $five = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."duobao_item where is_five=1 and has_lottery=1");
        print_r($five);die;
        foreach ($five as $k=>$v){
            $five[$k]['create_time']=date("Y-m-d H:i:s",$v['create_time']);
...
// 		var_dump($order_deal_items['is_fictitious']);exit;
       ...
       if($zq){
           $id = intval($_REQUEST['id']);
           $five = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."duobao_item where is_five=1 and has_lottery=1 and id=".$id);
//            print_r($five);die;
...
//       print_r($five[0]);die;
	...
	public function fa_huo(){
        $id = intval($_REQUEST['id']);
        $five = $GLOBALS['db']->getOne("select take_effect from ".DB_PREFIX."duobao_item where is_five=1 and has_lottery=1 and id=".$id);
//        print_r($five);die;
        if($five ==0){
