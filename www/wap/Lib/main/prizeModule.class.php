...
class prizeModule extends MainBaseModule
{
    ...
    public function index()
	{
        ...
        if ($user_login_status != LOGIN_STATUS_LOGINED) {
            echo "<script>window.location.href='http://www.gagoods.cn/wap/index.php?ctl=user&act=login&show_prog=1'</script>";
            die;
        }
        $restaraunts = [];
...
//        var_dump($count);die;
        $GLOBALS['tmpl']->assign('date11',$date11);
    ...
    foreach ($proArr as $key => $proCur) {
     ...
     public function ajax(){
         ...
         if($rid == 3){
             ...
             $key = $rid-1;
            $money = $award[$key];
            $GLOBALS['db']->query("update ".DB_PREFIX."user set give_money = give_money+".$money.",can_use_give_money = can_use_give_money+".$money." where id = ".$user_info['id']);

            //添加大转盘资金日志
             $give_log['log_info'] = '参加大转盘活动获得赠送金额'.$money;
             $give_log['log_time'] = time();
             $give_log['log_user_id'] = $user_info['id'];
             $give_log['money'] = $money;
             $give_log['user_id'] = $user_info['id'];
