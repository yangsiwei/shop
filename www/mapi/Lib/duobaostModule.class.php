...
class duobaostApiModule extends MainBaseApiModule{
    ...
    //	if($min_buy==0)$min_buy=1;
        ...
        $sql_count = "SELECT count(*)
    		        ...
    		        WHERE
    		            is_effect = 1 AND
    		            success_time = 0 and is_pk=0 and is_topspeed=0 and is_coupons=0  and is_number_choose=0 and (min_buy = ".$min_buy." or unit_price=".$min_buy.") ";
        if($cate_item)
        {
        ...
        $sql = "SELECT id, name, max_buy,min_buy,unit_price, current_buy, (max_buy-current_buy) as surplus_buy, icon
                    FROM
                    	".DB_PREFIX."duobao_item
    		        WHERE
    		            is_effect = 1 AND is_coupons=0 and success_time = 0 and is_pk=0 and is_topspeed=0 and is_number_choose=0 and (min_buy = ".$min_buy." or unit_price=".$min_buy.") ";
        if($cate_item)
        {
