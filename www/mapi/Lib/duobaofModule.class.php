 ...
 * Time: 15:42
 */
class duobaofApiModule extends MainBaseApiModule{
    /**
     * xx元全部商品列表接口
     * 输入：
     * data_id: int 分页ID
    ...
    public function index(){
        ...
        $sql_count = "SELECT count(*)
    		        ...
    		        WHERE
    		            is_effect = 1 AND
    		            success_time = 0 and is_pk=0 and is_topspeed=0 and is_coupons=0  and is_number_choose=0 and fair_type='five' has_lottery=0 and progress < 100 and is_five=1 ";
        if($cate_item)
        {
        ...
        $sql = "SELECT id, name, max_buy,min_buy,unit_price, current_buy, (max_buy-current_buy) as surplus_buy, icon
                    FROM
                    	".DB_PREFIX."duobao_item
    		        WHERE
    		            is_effect = 1  and has_lottery=0 and progress < 100 and fair_type='five' and is_five=1";
        if($cate_item)
        {
