<?php
class topspeedApiModule extends MainBaseApiModule{
    /**
     * 极速专区全部商品列表接口
     * 输入：
     * data_id: int 分页ID
    ...
    public function index(){
        ...
        $sql_count = "SELECT count(*)
    		        ...
    		        WHERE
    		            is_effect = 1 AND
    		            success_time = 0 and is_topspeed=1 and is_pk=0 and is_number_choose=0 and is_coupons=0 ";
        if($cate_item)
        {
        ...
        $sql = "SELECT id, name,is_topspeed,unit_price, max_buy,min_buy, current_buy, (max_buy-current_buy) as surplus_buy, icon
                    FROM
                    	".DB_PREFIX."duobao_item
    		        WHERE
    		            is_effect = 1 AND success_time = 0 and is_topspeed=1 and is_pk=0 and is_number_choose=0 and is_coupons=0 ";
        if($cate_item)
        {
