...
class duobaosApiModule extends MainBaseApiModule{
    ...
    public function index(){
        ...
        $sql_count = "SELECT count(*)
    		        ...
    		        WHERE
    		            is_effect = 1 AND is_pk=0 and is_coupons=0 and
    		            success_time = 0 and is_number_choose=0";
        if($cate_item)
        {
        ...
        $sql = "SELECT id,unit_price, name,is_topspeed, max_buy, min_buy,current_buy, (max_buy-current_buy) as surplus_buy, icon
                    FROM
                    	".DB_PREFIX."duobao_item
    		        WHERE
    		            is_effect = 1 AND  is_pk=0 and is_coupons=0 and success_time = 0 and is_number_choose=0 ";
        if($cate_item)
        {
