...
$payment_lang = array(
    ...
    'psw'               =>  'API密钥', // 签名需要的key
    'android_appid'             =>  'android应用号',   //sdk支付所用
    'ios_appid'             =>  'ios应用号',   //sdk支付所用
);
$config = array(
...
class Jubyapp_payment implements payment {
    public function __construct(){
        $_GET['pay_class_name'] = 'Jubyapp';
    }
	...
	public function get_payment_code($payment_notice_id)
	{
		...
		$pay['pay_action'] = SITE_DOMAIN.APP_ROOT."/cgi/payment/Jubyapp/redirect.php?notice_id=".$payment_notice_id."&from=".$GLOBALS['request']['from'];
		$pay['payment_name'] = "聚宝云SDK支付";
		$pay['pay_money'] = $money;
		$pay['class_name'] = "Jubyapp";
		...
		else
		{
 		...
 		$notify_url = SITE_DOMAIN.APP_ROOT.'/callback/payment/Jubyapp_notify.php';
...
//         }
        ...
        {
            $pay['sdk_code'] = array(
                "pay_sdk_type"=>"jubaopay",
                "config"=>
                array(
                    "appid"=>$config['android_appid'],                          //AppId应用号
                    "amount"=>$money,                                   //金额(必需)
                    "goodsName"=>$title_name,                           //商品名称(必需)
                    "payid"=>$payment_notice['notice_sn'],              //商户订单号(必需)
                    "playerId"=>$payment_notice['user_id'],             //玩家Id(必需)
        ...
        if($agent_arr['sdk_type']=="ios")
        {
            $pay['sdk_code'] = array(
                ...
                "config"=>
                array(
                    "appid"=>$config['ios_appid'],                          //AppId应用号
                    "amount"=>$money,                                   //金额(必需)
                    "goodsName"=>$title_name,                           //商品名称(必需)
                    "payid"=>$payment_notice['notice_sn'],              //商户订单号(必需)
                    "playerId"=>$payment_notice['user_id'],             //玩家Id(必需)
	...
	public function notify($request){
	    ...
	    $payment_info = $GLOBALS['db']->getOne("select config from ".DB_PREFIX."payment where class_name = 'Jubyapp'");
	...
	public function get_display_code(){
	    ...
	    if(isMobile() || IS_MOBILE == 1){
	        ...
	        $payment_item = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."payment where class_name='Jubyapp'");
	        if($payment_item)
	        {
	            ...
	            {
	                $html .= "<img src='".APP_ROOT.$payment_item['logo']."' />聚宝云SDK支付 ";
	            }
	            else
	            {
	                $html .= '聚宝云SDK支付';
